...
def main():
    try:
        ...
        # Create S3 client
        # run aws configure, it will automatically use the credentials from the environment variables set in .env file
        bucket = os.getenv("S3_BUCKET_NAME") 
        s3 = s3_connection.s3_operations( bucket_name=bucket)
        df = s3.fetch_file_from_s3("data.csv") # load data from s3 bucket
